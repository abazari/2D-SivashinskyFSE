...
# Example 3: Random initial condition with size 128x128
noise = 0.25
u0 = 0.5
rng = np.random.default_rng(12345)  # the seed of random numbers generator
u0_128x128 = u0 + noise*rng.standard_normal([128,128]) 
...
for l in range(5): # l = 0 for N=8 and l=1 for N=16  and ... to l=4 for N=128
    ...
    # Example 3: Random initial condition with size N[l]xN[l]
    u[0] = u0_128x128[::2**(4-l),::2**(4-l)] 
    ...
    for i in range(1,Nsteps+1):
        ...
        if l == 4:
            ...
            if i in (400,1200,2400,4000,8000,12000,16000,24000,32000,48000,64000,96000,128000):
                fig = plt.figure(figsize=plt.figaspect(0.5))
                ax = fig.add_subplot(1, 2, 1)
